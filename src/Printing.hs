...
import Generator
    ( ParEl(..),
      ...
      Environment(..),
      ArgV(..),
      Argument(Argument, name), VerbMode (NoVerb, VerbIndent) )
...
texDocElement opts defs math (DocEnvironment Environment{begin, end, args, innerMath, innerVerb} argvs els)
...
texDocElement _ _ _ (DocVerb noInd txts) = P.vsep $ pretty <$> txts
